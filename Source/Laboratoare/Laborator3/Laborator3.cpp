...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	/*if (tx1 < 150) {
		tx1 += deltaTimeSeconds * 100;
	}
	if (ty1 < 250) {
		ty1 += deltaTimeSeconds * 100;
	}*/
	
	// modelMatrix *= Transform2D::Scale(sx, sy);
	//modelMatrix *= Transform2D::Translate(meshes["square1"]->positions.at(0).x, meshes["square1"]->positions.at(0).y);
	modelMatrix *= Transform2D::Rotate(angularStep);
	//modelMatrix *= Transform2D::Translate(-1 * (meshes["square1"]->positions.at(0).x), -1 * (meshes["square1"]->positions.at(0).y));


	modelMatrix *= Transform2D::Translate(650, 200);
	modelMatrix *= Transform2D::Rotate(angularStep);
	...
	modelMatrix = glm::mat3(1);
	/*if (tx2 < 400) {
		tx2 += deltaTimeSeconds * 100;
	}
	if (ty2 < 250) {
		ty2 += deltaTimeSeconds * 100;
	}*/
	...
	modelMatrix = glm::mat3(1);
	/*if (tx3 < 650) {
		tx3 += deltaTimeSeconds * 100;
	}
	if (ty3 < 250) {
		ty3 += deltaTimeSeconds * 100;
	}*/
