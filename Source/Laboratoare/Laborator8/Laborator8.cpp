...
void Laborator8::Update(float deltaTimeSeconds)
{
	...
	lightDirection = glm::vec3(0, -1, 0);
	rot = glm::rotate(rot, rotateY, glm::vec3(0, 1, 0));
	rot = glm::rotate(rot, rotateX, glm::vec3(1, 0, 0));
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint lType = glGetUniformLocation(shader->program, "light_type");
	glUniform1i(lType, lightType);

	GLint cutOffLoc = glGetUniformLocation(shader->program, "cut_off");
	glUniform1f(cutOffLoc, cutOffAngle);
