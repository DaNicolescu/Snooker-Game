...
{
	{
		...
		RenderSimpleMesh(1, meshes["sphere"], shaders["ShaderLab9"], modelMatrix, mapTextures["earth"]);
	}

	{
		...
		RenderSimpleMesh(0, meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["crate"], mapTextures["earth"]);
	}

	{
		...
		RenderSimpleMesh(0, meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["random"]);
	}

	{
		...
		modelMatrix = glm::toMat4(GetSceneCamera()->transform->GetWorldRotation());
		modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0f, 0.5f, 0.0f));
		RenderSimpleMesh(0, meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"]);
	}

	{
		...
		RenderSimpleMesh(0, meshes["bamboo"], shaders["ShaderLab9"], modelMatrix, mapTextures["bamboo"]);
	}
}
...
void Laborator9::RenderSimpleMesh(int earth, Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	int loc_earth = glGetUniformLocation(shader->program, "earth");
	glUniform1i(loc_earth, earth);
