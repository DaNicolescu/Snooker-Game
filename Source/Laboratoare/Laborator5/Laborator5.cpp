...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix *= glm::translate(modelMatrix, glm::vec3(-2, 3.5f, 0));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_W)) {
			...
			camera->TranslateForward(20 * deltaTime);
		}

		if (window->KeyHold(GLFW_KEY_A)) {
			// TODO : translate the camera to the left
			camera->TranslateRight(-20 * deltaTime);
		}
		...
		if (window->KeyHold(GLFW_KEY_Q)) {
			...
			camera->TranslateUpword(-20 * deltaTime);
		}
	...
	if (window->KeyHold(GLFW_KEY_K)) {
		radians -= 2;
		projectionMatrix = glm::perspective(RADIANS(radians), window->props.aspectRatio, 0.01f, 100.0f);
	}
	...
	if (window->KeyHold(GLFW_KEY_N)) {
		right -= 2;
		projectionMatrix = glm::ortho(-10.0f, right, -10.0f, 10.0f, 0.1f, 200.0f);
	}
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_O) {
		projectionMatrix = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 0.1f, 200.0f);
	}
	if (key == GLFW_KEY_P) {
