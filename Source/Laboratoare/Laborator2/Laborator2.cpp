...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vector<VertexFormat> verticesT
		{
			...
			VertexFormat(glm::vec3(0.3f, 1, 0.3f), glm::vec3(0, 1, 0)),
		...
		verticesC.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 0)));
		for (int i = 0; i < 20; i++) {
			verticesC.push_back(VertexFormat(glm::vec3(cos(18 * i * 3.14 / 180), sin(18 * i * 3.14 / 180), 0), glm::vec3(1, 0, 0)));
		}
		...
		for (int i = 0; i < 19; i++) {
			...
			indicesC.push_back(i + 2);
		}

		indicesC.push_back(0);
		indicesC.push_back(20);
		...
		Mesh *tetr = CreateMesh("tetr", verticesT, indicesT);
		Mesh *sq = CreateMesh("sq", verticesP, indicesP);
		Mesh *circ = CreateMesh("circ", verticesC, indicesC);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["sq"], shaders["VertexColor"], glm::vec3(1, 0, 0), glm::vec3(0.25f));
	RenderMesh(meshes["circ"], shaders["VertexColor"], glm::vec3(2, 2, 2), glm::vec3(0.25f));
