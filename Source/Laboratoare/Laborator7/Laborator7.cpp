...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint eyeLoc = glGetUniformLocation(shader->program, "eye_position");
	...
	GLint kdLoc = glGetUniformLocation(shader->program, "material_kd");
	glUniform1f(kdLoc, materialKd);

	GLint ksLoc = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(ksLoc, materialKs);

	GLint shineLoc = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(shineLoc, materialShininess);

	/*GLint objColorLoc = glGetUniformLocation(shader->program, "object_color");
	glUniform3fv()*/
